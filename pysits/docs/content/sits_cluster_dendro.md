...
`sits_cluster_dendro()` takes time series and produces a `SITSTimeSeriesModel`
with an added "cluster" column. The function first calculates a dendrogram and
...
`sits_cluster_dendro()`.
`sits_cluster_clean()` takes time series that have an additional `cluster`
produced by `sits_cluster_dendro()` and removes labels that are minority in
each cluster.
...
Returns:
    SITSTimeSeriesModel: time series with an added "cluster" column.
